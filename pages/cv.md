...
- **职责描述：**
  - **数据流程：** 根据曝光日志、特征日志、行为日志等数据源，使用Hadoop和Spark大数据框架，构建全面的数据处理流程，包括清洗数据和ETL工程。通过提取和关联特征和样本信息，最终建立一个高效的数据仓库。确保数据的高质量和一致性，为后续的特征工程和模型训练提供坚实的数据基础。
  - **样本指标：** 结合实际业务，规范标签体系。增加负向行为指标（如拒绝），细化正向行为指标（如已读），提高指标的全面性和准确性。新的标签体系更新到BI系统；在模型训练的过程中，同步更新样本标签，保证模型的准确性和稳定性。
  - **特征工程：** 
  - **特征选择：** 
