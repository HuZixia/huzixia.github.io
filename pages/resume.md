...
<div style="display: flex; justify-content: space-between;">
  <div style="line-height: 1.2; margin: 0;">
    <p><strong>金融AI方案设计与实现：</strong>深耕信贷云场景金融核心业务，从 0 到 1 搭建覆盖撞库、进件、提现全流程的智能路由系统，通过精准匹配用户与机构需求优化资源分配，直接支撑榕树业务放款增量超 2 亿元、场景金融业务增量突破 1000 万元；同步沉淀可复用的技术框架与增长策略，为后续业务规模化扩张预留优化空间，具备金融场景痛点拆解、技术方案落地及收益转化的全链路能力</p>
    <p><strong>大模型技术深度应用：</strong>主导大模型微调（LoRA/QLora/PEFT）、RAG 技术集成（LangChain/Chromadb）、Agent 协作机制落地，在智能营销（短信文案生成）、智能客服（知识库检索/表格处理）、视频生成等场景实现个性化需求匹配，理解大模型技术边界与应用逻辑</p>
    <p><strong>POC流程与团队协作：</strong>跨产品、运营、研发等部门推进算法方案落地，设计实验指标与分析体系，通过 A/B 实验验证策略效果（如进件路由人均放款提升 3%-30%+），具备需求拆解、资源协调及问题快速响应能力</p>
    <p><strong>客户需求挖掘与转化：</strong>基于用户画像与数据分析构建精准推荐逻辑（如提现路由个性化推荐带来超 600 万放款增量），沉淀 “业务痛点 - 技术方案 - 效果量化” 闭环经验，可转化为客户需求洞察与解决方案设计能力</p>
    <p><strong>推荐算法规模化增长：</strong>主导店长直聘、BOSS直聘蓝领业务推荐系统全链路建设（召回/排序/重排），通过多目标学习（MTL）、强化学习等技术优化供需匹配，推动店长直聘B/C两端转化率分别提升超 100%和150%+，BOSS直聘蓝领业务转化率分别提升超 30%和50%+，蓝领交付业务入职转化率增幅达100%以上，实现核心业务指标规模化增长与用户体验双重提升</p>
...
- **项目简介：** 进件路由项目聚焦于进件渠道的智能分发与优化，通过构建推荐系统、多目标模型及精细化运营策略，实现渠道流量的精准分配，提升各渠道人均放款金额、毛利等核心指标，支撑业务规模化增长与效益提升。
<style>
p {
    margin-bottom: 2em; /* 或者使用其他值，根据需要调整 */
}
...
- **职责描述：**
  - **渠道智能分发：** 主导 20 余个进件渠道的算法模型接入与机构分发策略迭代，通过智能路由机制优化渠道分发，提升人均放款金额，为业务规模化增长奠定基础。
  - **推荐系统搭建：** 全流程主导推荐系统从 0 到 1 构建，攻克样本构建、特征深度挖掘、多目标建模等核心技术，驱动人均放款提升超 20%、人均毛利提升超 40%。
  - **流量精细运营：** 基于渠道特性差异化调优，在 10%-50% 流量配置区间内实现单渠道人均放款提升 3%-30%；通过精准分配，榕树业务放款增量超2亿元，场景金融业务增量超1000万元。
  - **特征模型优化：** 负责 10+ 个多目标进件模型全链路优化（累计上线 100 余次），提升决策精准度；开发 PID 维度等核心特征，强化模型表达能力；开发新老客实验指标与分析代码，助力模型迭代。
...
- **项目技术：** 个性化推荐算法 + 多场景模型+ 特征工程 + 推荐系统全链路机制（训练 → 发布 → 上线 → 校验） + 模型迭代策略
<style>
p {
    margin-bottom: 2em; /* 或者使用其他值，根据需要调整 */
}
</style>
- **项目简介：** 提现路由项目旨在优化提现环节的产品推荐效果，通过构建全流程推荐系统、个性化推荐模型及全面的特征体系，精准匹配用户需求与可提现产品，提升人均放款金额与业务增量价值。
<style>
p {
    margin-bottom: 2em; /* 或者使用其他值，根据需要调整 */
}
...
- **职责描述：**
  - **推荐系统搭建：** 主导推荐系统全链路体系搭建，打通模型训练、发布、自动上线及校验闭环，实现天级实验效果分析，提升策略迭代效率。
  - **个性推荐设计：** 针对可提现产品数量限制，设计算法模型实现个性化推荐，推动人均放款金额提升 3%-5%，累计带来放款增量超 600 万元。
  - **模型迭代优化：** 围绕提现环节完成 3 轮以上样本迭代，开发适配新老客及全机构场景的模型 8 个，覆盖多样化需求，夯实推荐效果。
  - **特征体系建设：** 累计开发特征 1000+，包括集团 DTS 特征 400+、pid 维度统计特征 400+、风控类特征 100+、实时特征 30 + 等，构建全面的特征支撑体系。
...
- **项目技术：** 推荐算法 + 特征工程 + 模型效能优化 + 全链路算法架构
<style>
p {
    margin-bottom: 2em; /* 或者使用其他值，根据需要调整 */
}
</style>
- **项目简介：** 整体撞库项目旨在提升场景撞库环节的效率与精准度，通过优化算法策略、完善数据链路及模型效能，为后续进件转化奠定技术基础，同时构建业务全链路的智能化算法应用体系，支撑业务流程的高效运转。
<style>
p {
    margin-bottom: 2em; /* 或者使用其他值，根据需要调整 */
}
...
- **职责描述：**
  - **算法方案的设计：** 设计场景整体撞库环节的推荐算法方案，优化撞库效率与精准度，为后续进件转化提供技术支撑。
  - **跨部门协同落地：** 深度联动产品、运营、研发等多部门，明确研发落地细节，协调资源对齐目标，保障方案从设计到执行的顺畅推进。
  - **模型与数据优化：** 推进样本特征开发，优化模型耗时；优化特征存储方式，提升数据获取效率与稳定性，强化模型底层支撑能力。
  - **全链路体系搭建：** 从业务线全局出发，构建撞库环节全流程算法模型体系，推动各方需求开发，支撑业务链路的智能化升级。
...
- **项目技术：** 大模型（微调） + 用户画像技术 + 个性化推荐算法 + 营销文案生成模型 + 闭环运营链路（内容生成 → 推送 → 效果反馈）
<style>
p {
    margin-bottom: 2em; /* 或者使用其他值，根据需要调整 */
}
</style>
- **项目简介：** 智能营销项目聚焦于通过大模型与用户画像实现个性化营销，设计智能短信方案，构建营销内容生成与推送的闭环链路，输出个性化营销内容，支撑精细化运营，提升营销效果与用户响应率。
<style>
p {
    margin-bottom: 2em; /* 或者使用其他值，根据需要调整 */
}
...
- **职责描述：**
  - **智能营销设计：** 设计大模型驱动的个性化短信方案，融合模型微调和智能推荐策略，构建营销内容生成与推送的闭环链路。
  - **用户画像应用：** 结合用户画像开发大模型营销文案生成机制，输出个性化的营销内容，为精细化运营提供技术支撑，提升营销精准度与效果。
...
<div style="display: flex; justify-content: space-between;">
    <p style="font-size: 20px;"><strong>高级算法工程师 — 北京海纳金川科技有限公司</strong></p> 
